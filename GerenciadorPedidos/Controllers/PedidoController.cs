...
namespace GerenciadorPedidos.Controllers
{
    ...
    {   
        ...
        public IActionResult Update(int pedidoId, [FromBody] PedidoDto pedidoUpdate)
        {
            ...
            var pedido = _pedidoRepository.getALL().Where(x => x.Id == pedidoId).First();

            if (!ModelState.IsValid)
                return BadRequest();

            pedido.Descricao = pedidoUpdate.Descricao;
            pedido.Preco = pedidoUpdate.Preco;

            //var pedidoMap = _mapper.Map<Pedido>(pedidoUpdate);
